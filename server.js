...
// ── Consultants (candidates with active placements OR status "Placed") ──
app.get("/api/consultants", async (req, res) => {
  try {
    ...
    // 1. Get all active placements to find currently-placed candidate IDs
    var placData = await bhFetchAll("query/Placement", {
    ...
    var placements = placData.data || [];
    var placedCandIds = {};
    var placementsByCandidate = {};
    placements.forEach(function(p) {
      if (p.candidate && p.candidate.id) {
        placedCandIds[p.candidate.id] = true;
        if (!placementsByCandidate[p.candidate.id]) placementsByCandidate[p.candidate.id] = [];
        placementsByCandidate[p.candidate.id].push(p);
      }
    });

    // 2. Also get candidates with status "Placed" (catches any without active placement records)
    var placedStatusData = await bhFetchAll("search/Candidate", {
      query: 'isDeleted:0 AND status:"Placed"',
      fields: "id",
    });
    (placedStatusData.data || []).forEach(function(c) { placedCandIds[c.id] = true; });

    // 3. Fetch full candidate details for all placed candidates
    var allIds = Object.keys(placedCandIds).map(Number);
    if (allIds.length === 0) return res.json({ data: [], total: 0 });

    var candidates = [];
    for (var i = 0; i < allIds.length; i += 20) {
      var batch = allIds.slice(i, i + 20);
      var details = await Promise.all(batch.map(function(cid) {
        return bhFetch("entity/Candidate/" + cid, {
          fields: "id,firstName,lastName,occupation,status,address,email,phone,dateAvailable,customText1,customText2,customText5,customText6,customText7,customTextBlock1,owner,salary"
        }).then(function(r) { return r.data || r; }).catch(function() { return null; });
      }));
      details.forEach(function(c) { if (c && c.id) candidates.push(c); });
    }

    // 4. Build response — merge candidate + placement data
    var result = candidates.map(function(c) {
      var pList = placementsByCandidate[c.id] || [];
      var activePlac = pList[0]; // most recent
      return {
        id: c.id,
        firstName: titleCase(c.firstName),
        lastName: titleCase(c.lastName),
        ...
        owner: c.owner ? (c.owner.firstName + " " + c.owner.lastName) : "",
        // Placement info
        currentClient: activePlac && activePlac.jobOrder ? activePlac.jobOrder.title || "" : "",
        placementStatus: activePlac ? activePlac.status || "" : "",
        endDate: activePlac && activePlac.dateEnd ? new Date(activePlac.dateEnd).toLocaleDateString() : "",
        billRate: activePlac && activePlac.clientBillRate ? "$" + activePlac.clientBillRate + "/hr" : "",
        payRate: activePlac && activePlac.payRate ? "$" + activePlac.payRate + "/hr" : "",
      };
    });

    // 5. Apply search/cert filters
    if (q) {
      var ql = q.toLowerCase();
      result = result.filter(function(c) {
        return (c.firstName + " " + c.lastName).toLowerCase().indexOf(ql) >= 0 ||
          c.title.toLowerCase().indexOf(ql) >= 0 ||
          c.primaryCert.toLowerCase().indexOf(ql) >= 0;
      });
    }
    if (cert) {
      var cl = cert.toLowerCase();
      result = result.filter(function(c) {
        return c.primaryCert.toLowerCase().indexOf(cl) >= 0 || c.secondaryCert.toLowerCase().indexOf(cl) >= 0;
      });
    }
...
app.put("/api/candidates", async (req, res) => {
  try {
    ...
    const ALLOWED_FIELDS = [
      "firstName", "lastName", "middleName", "nickName", "name", "occupation",
      "status", "email", "email2", "phone", "phone2", "mobile",
      "salary", "hourlyRate", "dayRate", "dateAvailable",
      "employeeType", "source", "educationDegree", "companyName",
      "willRelocate", "travelLimit",
      "customText1", "customText2", "customText3", "customText4", "customText5",
      "customText6", "customText7", "customText8", "customText9", "customText10",
    ...
    for (const key of Object.keys(body)) {
      if (ALLOWED_FIELDS.includes(key)) {
        newCand[key] = body[key];
      }
    }
    // Handle owner
    if (body.owner && typeof body.owner === "object" && body.owner.id) {
      newCand.owner = { id: parseInt(body.owner.id) };
    } else if (body.ownerId) {
      newCand.owner = { id: parseInt(body.ownerId) };
    }
    // Handle address
    const addressFields = ["address1", "address2", "city", "state", "zip", "countryID"];
    const addrUpdates = {};
    ...
    if (hasAddr) newCand.address = addrUpdates;
    // Set name for Bullhorn
    newCand.name = ((body.firstName || "") + " " + (body.lastName || "")).trim();
    const result = await bhWrite("entity/Candidate", newCand, "PUT");
    console.log("[Create Candidate]", result);
    res.json({ success: true, message: "Candidate created", data: result });
  } catch (e) {
    console.error("[Create Candidate]", e.message);
    res.status(500).json({ error: e.message });
  }
});
...
app.put("/api/clients", async (req, res) => {
  try {
    ...
    for (const key of Object.keys(body)) {
      if (ALLOWED_FIELDS.includes(key)) {
        newClient[key] = body[key];
      }
    }
    // Handle owner
    if (body.owner && typeof body.owner === "object" && body.owner.id) {
      newClient.owner = { id: parseInt(body.owner.id) };
    } else if (body.ownerId) {
      newClient.owner = { id: parseInt(body.ownerId) };
    }
    // Handle address
    const addressFields = ["address1", "address2", "city", "state", "zip", "countryID"];
    const addrUpdates = {};
    ...
    if (hasAddr) newClient.address = addrUpdates;
    const result = await bhWrite("entity/ClientCorporation", newClient, "PUT");
    console.log("[Create Client]", result);
    res.json({ success: true, message: "Client created", data: result });
  } catch (e) {
...
// ── Update Opportunity ─────────────────────
app.post("/api/opportunities/:id/update", async (req, res) => {
  try {
    const oppId = parseInt(req.params.id);
    const updates = req.body;
    const ALLOWED_FIELDS = [
      "title", "status", "type", "dealValue", "weightedDealValue",
      "winProbabilityPercent", "estimatedStartDate", "estimatedEndDate",
      "description", "notes", "salary", "numOpenings",
      "customText1", "customText2", "customText3", "customText4", "customText5", "customText6",
      "customDate1", "customDate2", "customFloat1", "customFloat2",
    ];
    const safeUpdates = {};
    for (const key of Object.keys(updates)) {
      if (ALLOWED_FIELDS.includes(key)) {
    ...
    const result = await bhWrite(`entity/Opportunity/${oppId}`, safeUpdates, "POST");
    console.log("[Update Opportunity]", oppId, "→", Object.keys(safeUpdates), result);
    res.json({ success: true, message: "Opportunity updated", changedFields: Object.keys(safeUpdates) });
  } catch (e) {
    console.error("[Update Opportunity]", e.message);
    res.status(500).json({ error: e.message });
  }
});

// ── Create Opportunity ─────────────────────
app.put("/api/opportunities", async (req, res) => {
  try {
    const body = req.body;
    if (!body.title) {
    ...
    const newOpp = { status: body.status || "Identified", isDeleted: false };
    for (const key of Object.keys(body)) {
      if (ALLOWED_FIELDS.includes(key)) newOpp[key] = body[key];
    }
    if (body.ownerId) newOpp.owner = { id: parseInt(body.ownerId) };
    if (body.clientCorporationId) newOpp.clientCorporation = { id: parseInt(body.clientCorporationId) };
    const result = await bhWrite("entity/Opportunity", newOpp, "PUT");
    console.log("[Create Opportunity]", result);
    res.json({ success: true, message: "Opportunity created", data: result });
  } catch (e) {
    console.error("[Create Opportunity]", e.message);
    res.status(500).json({ error: e.message });
  }
});
...
// ── Sourcing Board — per-job candidate tracking ──────────────────────────

var SOURCING_STAGES = db.SOURCING_STAGES || ["identified","reached_out","talking","interviewing","collecting_refs","in_pipeline","submitted","passed"];
var SOURCING_STAGE_LABELS = {
  identified: "ID'd", reached_out: "Reached Out", talking: "Talking to Us",
  interviewing: "Interviewing", collecting_refs: "Collecting Refs",
  in_pipeline: "In Pipeline", submitted: "Submitted", passed: "Passed"
};

// Get all sourcing candidates for a job
app.get("/api/sourcing/:jobId", async (req, res) => {
  try {
    var jobId = parseInt(req.params.jobId);
    if (!jobId) return res.status(400).json({ error: "Invalid jobId" });

    if (!db.ready) return res.status(503).json({ error: "Database not enabled", data: [], stages: SOURCING_STAGES });

    var candidates = await db.getSourcingForJob(jobId);
    res.json({ data: candidates, total: candidates.length, stages: SOURCING_STAGES, stageLabels: SOURCING_STAGE_LABELS });
  } catch (e) {
    console.error("[Sourcing] Get error:", e.message);
    res.status(500).json({ error: e.message, data: [] });
  }
});

// Add a single candidate to sourcing board
app.post("/api/sourcing/:jobId", async (req, res) => {
  try {
    var jobId = parseInt(req.params.jobId);
    if (!jobId) return res.status(400).json({ error: "Invalid jobId" });
    if (!db.ready) return res.status(503).json({ error: "Database not enabled" });

    var user = getUser(req);
    var data = req.body;
    data.addedBy = user ? user.id : null;
    data.addedByName = user ? user.name : "System";

    var result = await db.addSourcingCandidate(jobId, data);
    console.log("[Sourcing] Added candidate to job", jobId, "→ ID", result.id);
    res.json({ success: true, id: result.id });
  } catch (e) {
    console.error("[Sourcing] Add error:", e.message);
    res.status(500).json({ error: e.message });
  }
});

// Bulk import candidates (from CSV or batch add)
app.post("/api/sourcing/:jobId/bulk-import", async (req, res) => {
  try {
    var jobId = parseInt(req.params.jobId);
    if (!jobId) return res.status(400).json({ error: "Invalid jobId" });
    if (!db.ready) return res.status(503).json({ error: "Database not enabled" });
    ...
    var result = await db.bulkAddSourcing(jobId, candidates, user ? user.id : null, user ? user.name : "System");
    console.log("[Sourcing] Bulk import to job", jobId, "— added:", result.added, "skipped:", result.skipped);
    res.json({ success: true, added: result.added, skipped: result.skipped });
  } catch (e) {
    console.error("[Sourcing] Bulk import error:", e.message);
    res.status(500).json({ error: e.message });
  }
});

// Auto-populate from Smart Match
app.post("/api/sourcing/:jobId/auto-populate", async (req, res) => {
  try {
    var jobId = parseInt(req.params.jobId);
    if (!jobId) return res.status(400).json({ error: "Invalid jobId" });
    if (!db.ready) return res.status(503).json({ error: "Database not enabled" });

    // Use the smart-match endpoint to find matching candidates
    var matchUrl = session.restUrl
      ? `${req.protocol}://${req.get("host")}/api/smart-match/${jobId}`
      : null;

    // Fetch smart match results internally
    var smRes = await new Promise(function(resolve, reject) {
      var http = require("http");
      var url = `http://localhost:${process.env.PORT || 3000}/api/smart-match/${jobId}`;
    ...
    if (!smRes.candidates || smRes.candidates.length === 0) {
      return res.json({ success: true, added: 0, skipped: 0, message: "No matching candidates found" });
    }

    var toImport = smRes.candidates.slice(0, 50).map(function(c) {
      return {
        candidateId: c.id,
        source: "bullhorn",
        stage: "identified",
    ...
    var user = getUser(req);
    var result = await db.bulkAddSourcing(jobId, toImport, user ? user.id : null, user ? user.name : "Smart Match");
    console.log("[Sourcing] Auto-populated job", jobId, "— added:", result.added, "skipped:", result.skipped);
    res.json({ success: true, added: result.added, skipped: result.skipped });
  } catch (e) {
    console.error("[Sourcing] Auto-populate error:", e.message);
    res.status(500).json({ error: e.message });
  }
});

// Update a single sourcing candidate
app.put("/api/sourcing/:id", async (req, res) => {
  try {
    ...
    var result = await db.updateSourcingCandidate(id, req.body);
    if (!result) return res.status(404).json({ error: "Not found" });
    console.log("[Sourcing] Updated", id);
    res.json({ success: true });
  } catch (e) {
    console.error("[Sourcing] Update error:", e.message);
    res.status(500).json({ error: e.message });
  }
});

// Remove a single sourcing candidate
app.delete("/api/sourcing/:id", async (req, res) => {
  try {
    ...
    await db.removeSourcingCandidate(id);
    console.log("[Sourcing] Removed", id);
    res.json({ success: true });
  } catch (e) {
    console.error("[Sourcing] Remove error:", e.message);
    res.status(500).json({ error: e.message });
  }
});

// Bulk remove
app.post("/api/sourcing/:jobId/bulk-remove", async (req, res) => {
  try {
    if (!db.ready) return res.status(503).json({ error: "Database not enabled" });
    ...
    var removed = await db.bulkRemoveSourcing(ids.map(Number));
    console.log("[Sourcing] Bulk removed", removed, "candidates");
    res.json({ success: true, removed: removed });
  } catch (e) {
    console.error("[Sourcing] Bulk remove error:", e.message);
    res.status(500).json({ error: e.message });
  }
});

// Bulk update stage
app.post("/api/sourcing/:jobId/bulk-stage", async (req, res) => {
  try {
    if (!db.ready) return res.status(503).json({ error: "Database not enabled" });
    var ids = req.body.ids || [];
    var stage = req.body.stage;
    if (!stage || SOURCING_STAGES.indexOf(stage) < 0) return res.status(400).json({ error: "Invalid stage" });
    var updated = await db.bulkUpdateSourcingStage(ids.map(Number), stage);
    console.log("[Sourcing] Bulk stage update →", stage, "for", updated, "candidates");
    res.json({ success: true, updated: updated });
  } catch (e) {
    console.error("[Sourcing] Bulk stage error:", e.message);
    res.status(500).json({ error: e.message });
  }
});

// Bulk toggle selected
app.post("/api/sourcing/:jobId/bulk-select", async (req, res) => {
  try {
    if (!db.ready) return res.status(503).json({ error: "Database not enabled" });
    var ids = req.body.ids || [];
    var isSelected = req.body.isSelected !== false;
    var updated = await db.bulkToggleSelected(ids.map(Number), isSelected);
    res.json({ success: true, updated: updated });
  } catch (e) {
    console.error("[Sourcing] Bulk select error:", e.message);
    res.status(500).json({ error: e.message });
  }
});
  ...
  try {
    ...
    try {
      ...
      topConsultants = (candData.data || []).map(function(c) {
        ...
        var availSoon = avail && avail.getTime() <= now + days35;
        return {
    ...
    try {
      if (db.ready) {
        ...
        var expData = await bhFetchAll("query/Placement", {
          where: "dateEnd IS NOT NULL AND dateEnd >= " + now + " AND dateEnd <= " + (now + days35) + " AND (employmentType IS NULL OR (employmentType <> 'Direct Hire' AND employmentType <> 'Permanent'))",
          fields: "id,candidate,jobOrder,status,dateBegin,dateEnd,payRate,clientBillRate,employmentType",
          orderBy: "dateEnd",
        });
    ...
    try {
      if (db.ready) {
        ...
        var startData = await bhFetchAll("query/Placement", {
          where: "dateBegin IS NOT NULL AND dateBegin >= " + (now - days7) + " AND dateBegin <= " + (now + days35),
...
/**
 * Known Epic certifications for matching against resume text.
 * Maps lowercase aliases → canonical names.
 */
const EPIC_CERTS = {
  ...
  "epiccare": "EpicCare", "epiccare ambulatory": "EpicCare Ambulatory",
  "epiccare inpatient": "EpicCare Inpatient",
  "optime": "OpTime", "grand central": "Grand Central",
  "hyperspace": "Hyperspace", "my chart": "MyChart", "mychart": "MyChart",
  "wisdom": "Wisdom", "healthy planet": "Healthy Planet",
  "care everywhere": "Care Everywhere", "welcome": "Welcome",
...
function parseResumeText(text) {
  ...
  const result = {
    ...
    zip: { value: "", confidence: 0 },
    skills: { value: [], confidence: 0 },
    epicCertifications: { value: [], confidence: 0 },
    epicRole: { value: "", confidence: 0 },
    grade: { value: "", confidence: 0 },
    education: { value: [], confidence: 0 },
    workHistory: { value: [], confidence: 0 },
  ...
  if (emailMatch) {
    result.email = { value: emailMatch[0].toLowerCase(), confidence: 0.98 };
  }
  ...
  for (const pp of phonePatterns) {
    ...
    if (pm) {
      ...
      if (digits.length >= 10 && digits.length <= 11) {
        result.phone = { value: pm[0].trim(), confidence: 0.95 };
        break;
      }
    }
  }
  ...
  for (const line of headerLines) {
    ...
    if (words.length >= 2 && words.length <= 4 && nameCandidate.length < 40) {
      ...
      if (words.length === 2) {
        result.firstName.confidence = 0.95;
        result.lastName.confidence = 0.95;
      }
      break;
    }
  }
  ...
  if (addrMatch) {
    ...
    const addrIdx = fullText.indexOf(addrMatch[0]);
    const before = fullText.substring(Math.max(0, addrIdx - 100), addrIdx);
    const streetMatch = before.match(/(\d+[^,\n]*(?:Street|St|Avenue|Ave|Road|Rd|Drive|Dr|Boulevard|Blvd|Lane|Ln|Court|Ct|Way|Circle|Cir|Place|Pl)[.,]?\s*(?:#\s*\d+|Suite\s*\d+|Apt\.?\s*\d+)?)/i);
    if (streetMatch) {
      result.address = { value: streetMatch[1].trim() + ", " + addrMatch[0], confidence: 0.90 };
    } else {
      result.address = { value: addrMatch[0], confidence: 0.85 };
    }
  }
  ...
  if (foundCerts.size > 0) {
    result.epicCertifications = {
      value: [...foundCerts].sort(),
      confidence: foundCerts.size === 1 ? 0.85 : 0.90,
    };
  }
  ...
  // ── SKILLS ──
  const commonSkills = [
    "SQL", "Python", "JavaScript", "Java", "C#", "C\\+\\+", "HTML", "CSS",
    "React", "Angular", "Node\\.js", "TypeScript", "AWS", "Azure", "GCP",
    "Docker", "Kubernetes", "Git", "Agile", "Scrum", "Jira", "Confluence",
    "HL7", "FHIR", "HIPAA", "ICD-10", "CPT", "Revenue Cycle",
    "Data Analytics", "Tableau", "Power BI", "Excel", "Crystal Reports",
    "Project Management", "Change Management", "Training", "Go-Live",
    "EHR", "EMR", "Clinical Workflow", "Orders", "Clinical Documentation",
    "Interoperability", "Integration", "API", "REST", "SOAP",
    "Active Directory", "Citrix", "VMware", "ServiceNow",
  ];
  const detectedSkills = [];
  for (const skill of commonSkills) {
    const skillRegex = new RegExp("\\b" + skill + "\\b", "i");
    if (skillRegex.test(fullText)) {
      detectedSkills.push(skill.replace(/\\\+/g, "+").replace(/\\\./g, "."));
    }
  }
  if (detectedSkills.length > 0) {
    result.skills = { value: detectedSkills, confidence: 0.85 };
  }

  // ── EDUCATION ──
  const eduEntries = [];
  const degreePatterns = [
    /(?:Bachelor(?:'s)?|B\.?S\.?|B\.?A\.?)\s+(?:of|in|,)\s+([A-Z][^\n,]{2,50})/gi,
    /(?:Master(?:'s)?|M\.?S\.?|M\.?A\.?|MBA)\s+(?:of|in|,)\s+([A-Z][^\n,]{2,50})/gi,
    /(?:Doctor(?:ate)?|Ph\.?D\.?|D\.?O\.?|M\.?D\.?)\s+(?:of|in|,)\s+([A-Z][^\n,]{2,50})/gi,
    /(?:Associate(?:'s)?|A\.?A\.?S?\.?)\s+(?:of|in|,)\s+([A-Z][^\n,]{2,50})/gi,
  ];
  const degreeLabels = ["Bachelor's", "Master's", "Doctorate", "Associate's"];
  const seenDegreePositions = new Set();
  degreePatterns.forEach((pat, i) => {
    let m;
    while ((m = pat.exec(fullText)) !== null) {
      // Deduplicate by position — avoid overlapping matches
      const posKey = Math.floor(m.index / 20); // bucket nearby matches
      if (seenDegreePositions.has(posKey)) continue;
      seenDegreePositions.add(posKey);
      const field = m[1] ? m[1].trim().replace(/\s+from\s+.*$/i, "").replace(/\s+at\s+.*$/i, "") : "";
      if (field.length > 2 && !field.match(/^(EDUCATION|EXPERIENCE|SKILLS|SUMMARY)/i)) {
        eduEntries.push({
  ...
  result.workHistory = { value: workEntries, confidence: workEntries.length > 0 ? 0.80 : 0 };

  // ── SUMMARY / OBJECTIVE ──
  const summaryMatch = fullText.match(/(?:Summary|Objective|Profile|About)\s*:?\s*\n([^\n](?:[^\n]*\n?){1,5})/i);
  if (summaryMatch) {
    result.summary = { value: summaryMatch[1].trim().substring(0, 500), confidence: 0.85 };
  }
 ...
 * Accepts multipart file (PDF or DOCX) or JSON { text: "..." } for pasted text.
 * Returns parsed candidate fields with confidence scores.
 */
app.post("/api/resume/parse", async (req, res) => {
  try {
    const contentType = req.headers["content-type"] || "";
    let resumeText = "";
    let originalFile = null; // { name, data, contentType }
    ...
    if (contentType.includes("multipart/form-data")) {
      ...
      if (filePart) {
        ...
        resumeText = textPart.data.toString("utf8");
      } else {
        return res.status(400).json({ error: "No file or text provided" });
      }
    } else {
      // JSON body with pasted text (already parsed by express.json() middleware)
      resumeText = (req.body && req.body.text) || "";
    }
    ...
    console.log("[Resume Parser] Parsed resume —", parsed.firstName.value, parsed.lastName.value, "| Certs:", parsed.epicCertifications.value.join(", ") || "none");

    // Check for existing candidate by email
    let existingCandidate = null;
    const parsedEmail = (parsed.email && parsed.email.value) ? parsed.email.value.trim() : "";
    if (parsedEmail) {
      try {
        ...
        if (searchResult && searchResult.data && searchResult.data.length > 0) {
          ...
          existingCandidate = {
            ...
            email: ec.email || "",
            phone: ec.phone || "",
            city: (ec.address && ec.address.city) || "",
            state: (ec.address && ec.address.state) || "",
            zip: (ec.address && ec.address.zip) || "",
            occupation: ec.occupation || "",
            status: ec.status || "",
            source: ec.source || "",
            primaryCerts: ec.customText1 || "",
        ...
        console.log("[Resume Parser] Duplicate search failed (non-blocking):", searchErr.message);
      }
    }
    ...
    console.error("[Resume Parser Error]", e.message);
    res.status(500).json({ error: "Failed to parse resume: " + e.message });
  }
});

/**
 * POST /api/resume/create-candidate
 * Takes parsed + user-edited resume data, creates a Candidate in Bullhorn.
 * Body: { candidate: { firstName, lastName, email, phone, ... }, originalFileName, originalFileBase64 }
 */
app.post("/api/resume/create-candidate", async (req, res) => {
  try {
    const body = req.body; // already parsed by express.json() middleware
    ...
    // Build Bullhorn Candidate entity
    const candidatePayload = {
      ...
      customText1: (c.epicCertifications || []).join(", "),  // Primary Certs
      customText2: c.secondaryCerts || "",                    // Secondary Certs
      customText5: c.epicRole || "",                          // Epic Role
      customText6: c.grade || "",                             // Grade
      occupation: c.title || "",
      skillList: (c.skills || []).join(", "),
    };

    // Add address if present
    if (c.city || c.state || c.zip) {
      candidatePayload.address = {};
      if (c.address) candidatePayload.address.address1 = c.address;
      if (c.city) candidatePayload.address.city = c.city;
      if (c.state) candidatePayload.address.state = c.state;
      if (c.zip) candidatePayload.address.zip = c.zip;
    }

    // Create candidate via Bullhorn API (PUT = create per Bullhorn convention)
    const result = await bhWrite("entity/Candidate", candidatePayload, "PUT");
    const newId = result.changedEntityId || result.id;
    console.log("[Resume Parser] Created candidate", c.firstName, c.lastName, "→ ID:", newId);

    // If we have a resume file, upload it to the new candidate
    if (body.originalFileBase64 && body.originalFileName && newId) {
      try {
        const fileBuf = Buffer.from(body.originalFileBase64, "base64");
        const bhBoundary = "----BhResumeUpload" + Date.now();
        const CRLF = "\r\n";
        const bhParts = [];
        bhParts.push(
          `--${bhBoundary}${CRLF}Content-Disposition: form-data; name="file"; filename="${body.originalFileName}"${CRLF}Content-Type: application/octet-stream${CRLF}${CRLF}`
        );
        bhParts.push(fileBuf);
        bhParts.push(CRLF);
        bhParts.push(`--${bhBoundary}${CRLF}Content-Disposition: form-data; name="externalID"${CRLF}${CRLF}resume-parse-${Date.now()}${CRLF}`);
        bhParts.push(`--${bhBoundary}${CRLF}Content-Disposition: form-data; name="fileType"${CRLF}${CRLF}Resume${CRLF}`);
        bhParts.push(`--${bhBoundary}${CRLF}Content-Disposition: form-data; name="name"${CRLF}${CRLF}${body.originalFileName}${CRLF}`);
        bhParts.push(`--${bhBoundary}--${CRLF}`);
        const bhBody = Buffer.concat(bhParts.map(p => typeof p === "string" ? Buffer.from(p) : p));
        const s = await authenticate();
        const bhUrl = `${s.restUrl}file/Candidate/${newId}?BhRestToken=${s.bhRestToken}`;
        await fetch(bhUrl, {
          method: "PUT",
          headers: { "Content-Type": `multipart/form-data; boundary=${bhBoundary}` },
          body: bhBody,
        });
        console.log("[Resume Parser] Attached resume file to candidate", newId);
      } catch (fileErr) {
        console.error("[Resume Parser] File upload failed (candidate created):", fileErr.message);
      }
    }

    // Add education entries if available
    if (c.education && c.education.length > 0 && newId) {
      for (const edu of c.education) {
        try {
          await bhWrite("entity/CandidateEducation", {
            candidate: { id: newId },
            school: edu.school || "",
            degree: edu.degree || "",
            major: edu.field || "",
          ...
          console.error("[Resume Parser] Education entry failed:", eduErr.message);
        }
      }
    }

    // Add work history entries if available
    if (c.workHistory && c.workHistory.length > 0 && newId) {
      for (const job of c.workHistory) {
        try {
          await bhWrite("entity/CandidateWorkHistory", {
            candidate: { id: newId },
          ...
          console.error("[Resume Parser] Work history entry failed:", whErr.message);
        }
      }
    }
    ...
    res.json({
      success: true,
      candidateId: newId,
      message: `Candidate ${c.firstName} ${c.lastName} created successfully`,
    });
  } catch (e) {
    console.error("[Resume Parser Create Error]", e.message);
    res.status(500).json({ error: "Failed to create candidate: " + e.message });
  }
});

/**
 * POST /api/resume/update-candidate
 * Updates an existing candidate in Bullhorn with merged data from resume parse.
 * Body: { candidateId, candidate: { ... }, originalFileName, originalFileBase64 }
 */
app.post("/api/resume/update-candidate", async (req, res) => {
  try {
    const body = req.body;
    const c = body.candidate;
    ...
    // Build update payload (POST = update in Bullhorn)
    const updatePayload = {
      firstName: c.firstName,
      lastName: c.lastName,
      name: c.firstName + " " + c.lastName,
      email: c.email || "",
      phone: c.phone || "",
      customText1: (c.epicCertifications || []).join(", "),
      customText2: c.secondaryCerts || "",
      customText5: c.epicRole || "",
      customText6: c.grade || "",
      occupation: c.title || "",
      skillList: (c.skills || []).join(", "),
    };
    ...
    if (c.city || c.state || c.zip) {
      ...
      if (c.address) updatePayload.address.address1 = c.address;
      if (c.city) updatePayload.address.city = c.city;
      if (c.state) updatePayload.address.state = c.state;
      if (c.zip) updatePayload.address.zip = c.zip;
    }

    // Update candidate via Bullhorn API (POST = update)
    await bhWrite("entity/Candidate/" + candidateId, updatePayload, "POST");
    console.log("[Resume Parser] Updated existing candidate", c.firstName, c.lastName, "ID:", candidateId);

    // Upload resume file if provided
    if (body.originalFileBase64 && body.originalFileName) {
      try {
        const fileBuf = Buffer.from(body.originalFileBase64, "base64");
        const bhBoundary = "----BhResumeUpload" + Date.now();
        const CRLF = "\r\n";
        ...
        bhParts.push(fileBuf);
        bhParts.push(CRLF);
        bhParts.push(`--${bhBoundary}${CRLF}Content-Disposition: form-data; name="externalID"${CRLF}${CRLF}resume-update-${Date.now()}${CRLF}`);
        bhParts.push(`--${bhBoundary}${CRLF}Content-Disposition: form-data; name="fileType"${CRLF}${CRLF}Resume${CRLF}`);
        bhParts.push(`--${bhBoundary}${CRLF}Content-Disposition: form-data; name="name"${CRLF}${CRLF}${body.originalFileName}${CRLF}`);
        bhParts.push(`--${bhBoundary}--${CRLF}`);
        const bhBody = Buffer.concat(bhParts.map(p => typeof p === "string" ? Buffer.from(p) : p));
        const s = await authenticate();
        const bhUrl = `${s.restUrl}file/Candidate/${candidateId}?BhRestToken=${s.bhRestToken}`;
        await fetch(bhUrl, {
          method: "PUT",
          headers: { "Content-Type": `multipart/form-data; boundary=${bhBoundary}` },
          body: bhBody,
        });
        console.log("[Resume Parser] Attached resume file to candidate", candidateId);
      } catch (fileErr) {
        console.error("[Resume Parser] File upload failed (candidate updated):", fileErr.message);
      }
    }

    // Add new education entries if provided
    ...
    // Add new work history entries if provided
    ...
    res.json({
      ...
      message: `Candidate ${c.firstName} ${c.lastName} updated successfully`,
    });
  } catch (e) {
    console.error("[Resume Parser Update Error]", e.message);
    res.status(500).json({ error: "Failed to update candidate: " + e.message });
  }
});
...
app.get("/api/outlook/history/:entityType/:entityId", async (req, res) => {
  try {
    ...
    // ── 1. DB email_log (previously logged emails) ──
    var dbEmails = [];
    if (db.ready) {
      try {
        dbEmails = await db.getAll(
    ...
    // Always check Bullhorn notes API too (catches notes not yet in local DB)
    if (entityType === "candidate") {
      try {
        ...
        // Merge API notes with DB notes, deduping by id
        var seenNoteIds = {};
        bhNotes.forEach(function(n) { seenNoteIds[n.id] = true; });
        bhApiNotes.forEach(function(n) {
          if (!seenNoteIds[n.id]) { bhNotes.push(n); seenNoteIds[n.id] = true; }
        });
      } catch (bhNoteErr) { console.log("[Email History] Bullhorn note search failed:", bhNoteErr.message); }
    }

    // ── 3. Search ALL connected Outlook accounts for this candidate's emails ──
    var outlookEmails = [];
    try {
      ...
      if (entityEmail || entityEmail2) {
        var connectedUsers = Object.keys(_outlookUsers);
        // Also load from DB if in-memory is empty
        if (connectedUsers.length === 0 && db.ready) {
          try {
            var tokenRows = await db.getAll("SELECT email FROM outlook_tokens WHERE revoked = false");
            tokenRows.forEach(function(r) { if (connectedUsers.indexOf(r.email) === -1) connectedUsers.push(r.email); });
          } catch (e) {}
        }
        ...
        if (connectedUsers.length > 0) {
          ...
          // Search EVERY connected Outlook account
          for (var u = 0; u < connectedUsers.length; u++) {
            var acctEmail = connectedUsers[u];
            for (var i = 0; i < searchTerms.length; i++) {
              try {
                ...
                var searchData = await graphFetch(acctEmail, searchEndpoint);
                (searchData.value || []).forEach(function(msg) {
                  outlookEmails.push({
                    message_id: msg.id,
                    outlook_user: acctEmail,
              ...
              } catch (searchErr) { console.log("[Email History] Outlook search failed for " + searchTerms[i] + " in " + acctEmail + ":", searchErr.message); }
            }
          }
        }
      }
    } catch (outlookErr) { console.log("[Email History] Outlook search failed:", outlookErr.message); }

    // ── 4. Merge all email sources and dedupe ──
    var allEmails = [].concat(dbEmails, outlookEmails);
    var seenMsgIds = {};
    var mergedEmails = allEmails.filter(function(e) {
      var key = e.message_id || (e.outlook_user + "|" + e.subject + "|" + e.received_at);
      if (seenMsgIds[key]) return false;
      seenMsgIds[key] = true;
      return true;
    });
    // Sort newest first
...
// ═══ TAG SYSTEM (dashboard-only, predefined tags for candidates) ═══

// Get all predefined tags
app.get("/api/tags", async (req, res) => {
  try {
    if (!db.ready) return res.json({ tags: [] });
    var tags = await db.getAll("SELECT * FROM tags ORDER BY sort_order, name");
    for (var i = 0; i < tags.length; i++) {
      var countRow = await db.getOne("SELECT COUNT(*) as count FROM candidate_tags WHERE tag_id = $1", [tags[i].id]);
      tags[i].candidateCount = parseInt(countRow.count) || 0;
    }
    res.json({ tags: tags });
  } catch (e) {
...
// Create a new predefined tag (case-insensitive duplicate check)
app.post("/api/tags", async (req, res) => {
  try {
    if (!db.ready) return res.status(503).json({ error: "Database not ready" });
    var name = (req.body.name || "").trim();
    var color = req.body.color || "#6366f1";
    if (!name) return res.status(400).json({ error: "Tag name is required" });
    if (name.length > 50) return res.status(400).json({ error: "Tag name must be 50 characters or less" });
    var existing = await db.getOne("SELECT id, name FROM tags WHERE LOWER(name) = LOWER($1)", [name]);
    if (existing) return res.status(409).json({ error: "A tag named \"" + existing.name + "\" already exists" });
    var countRow = await db.getOne("SELECT COUNT(*) as count FROM tags");
    if (parseInt(countRow.count) >= 50) return res.status(400).json({ error: "Maximum of 50 tags reached. Delete unused tags before adding new ones." });
    var maxOrder = await db.getOne("SELECT COALESCE(MAX(sort_order), 0) as max_order FROM tags");
    var result = await db.query(
      "INSERT INTO tags (name, color, sort_order) VALUES ($1, $2, $3) RETURNING *",
      [name, color, (parseInt(maxOrder.max_order) || 0) + 1]
    );
...
app.put("/api/tags/:id", async (req, res) => {
  try {
    ...
    if (req.body.name !== undefined) {
      ...
      if (!name) return res.status(400).json({ error: "Tag name is required" });
      if (name.length > 50) return res.status(400).json({ error: "Tag name must be 50 characters or less" });
      var existing = await db.getOne("SELECT id, name FROM tags WHERE LOWER(name) = LOWER($1) AND id != $2", [name, tagId]);
      if (existing) return res.status(409).json({ error: "A tag named \"" + existing.name + "\" already exists" });
      updates.name = name;
    }
    ...
    params.push(tagId);
    await db.query("UPDATE tags SET " + setClauses.join(", ") + " WHERE id = $" + idx, params);
    res.json({ success: true });
  } catch (e) {
    console.error("[Tags] Update:", e.message);
    res.status(500).json({ error: e.message });
  }
});
...
app.post("/api/tags/merge", async (req, res) => {
  try {
    ...
    if (!sourceId || !targetId || sourceId === targetId) return res.status(400).json({ error: "Need different source and target tag IDs" });
    await db.query(
      "INSERT INTO candidate_tags (candidate_id, tag_id, tagged_by, tagged_at) " +
      "SELECT candidate_id, $1, tagged_by, tagged_at FROM candidate_tags WHERE tag_id = $2 " +
      "ON CONFLICT (candidate_id, tag_id) DO NOTHING",
      [targetId, sourceId]
    );
    await db.query("DELETE FROM candidate_tags WHERE tag_id = $1", [sourceId]);
    await db.query("DELETE FROM tags WHERE id = $1", [sourceId]);
    res.json({ success: true, message: "Tag merged successfully" });
  } catch (e) {
    console.error("[Tags] Merge:", e.message);
    res.status(500).json({ error: e.message });
  }
});
...
app.delete("/api/tags/:id", async (req, res) => {
  try {
    if (!db.ready) return res.status(503).json({ error: "Database not ready" });
    var tagId = parseInt(req.params.id);
    await db.query("DELETE FROM candidate_tags WHERE tag_id = $1", [tagId]);
...
app.get("/api/candidates/:id/tags", async (req, res) => {
  try {
    if (!db.ready) return res.json({ tags: [] });
    var candidateId = parseInt(req.params.id);
    var tags = await db.getAll(
      "SELECT t.*, ct.tagged_by, ct.tagged_at FROM candidate_tags ct JOIN tags t ON t.id = ct.tag_id WHERE ct.candidate_id = $1 ORDER BY t.sort_order, t.name",
      [candidateId]
    );
    res.json({ tags: tags });
  } catch (e) {
    console.error("[Tags] Candidate tags:", e.message);
    res.status(500).json({ error: e.message });
  }
});

// Add a tag to a candidate
app.post("/api/candidates/:id/tags", async (req, res) => {
  try {
    if (!db.ready) return res.status(503).json({ error: "Database not ready" });
    var candidateId = parseInt(req.params.id);
    var tagId = parseInt(req.body.tagId);
    var taggedBy = req.body.taggedBy || "Unknown";
    if (!tagId) return res.status(400).json({ error: "tagId is required" });
    await db.query(
      "INSERT INTO candidate_tags (candidate_id, tag_id, tagged_by) VALUES ($1, $2, $3) ON CONFLICT (candidate_id, tag_id) DO NOTHING",
      [candidateId, tagId, taggedBy]
    );
    res.json({ success: true });
  } catch (e) {
    console.error("[Tags] Add to candidate:", e.message);
    res.status(500).json({ error: e.message });
  }
});

// Remove a tag from a candidate
app.delete("/api/candidates/:id/tags/:tagId", async (req, res) => {
  try {
    if (!db.ready) return res.status(503).json({ error: "Database not ready" });
    var candidateId = parseInt(req.params.id);
    var tagId = parseInt(req.params.tagId);
    await db.query("DELETE FROM candidate_tags WHERE candidate_id = $1 AND tag_id = $2", [candidateId, tagId]);
    res.json({ success: true });
  } catch (e) {
    console.error("[Tags] Remove from candidate:", e.message);
    res.status(500).json({ error: e.message });
  }
});

// Get all candidates with a specific tag
app.get("/api/candidates/by-tag/:tagId", async (req, res) => {
  try {
    if (!db.ready) return res.json({ candidates: [] });
    var tagId = parseInt(req.params.tagId);
    var candidates = await db.getAll(
      "SELECT c.id, c.first_name, c.last_name, c.name, c.email, c.status, c.custom_text1, c.custom_text5, c.custom_text6, c.address_state, c.date_available, ct.tagged_by, ct.tagged_at " +
      "FROM candidate_tags ct JOIN candidates c ON c.id = ct.candidate_id WHERE ct.tag_id = $1 ORDER BY c.last_name, c.first_name",
      [tagId]
    );
    res.json({ candidates: candidates });
  } catch (e) {
    console.error("[Tags] By tag:", e.message);
    res.status(500).json({ error: e.message });
  }
});
...
app.get("/api/ask", async (req, res) => {
  try {
    ...
    if (howManyCertMatch) {
      ...
      const r = await bhFetchAll("query/Placement", { where: `(status = 'Approved' OR status = 'Actively On Contract') AND dateEnd >= ${nowMs}`, fields: "id", });
      answer = `You have **${r.total}** active placements.`;

    } else if (question.match(/(?:show|list|all)\s*consultant|placed\s+candidate|who.s\s+placed|on\s+contract|active\s+engagement/)) {
      const r = await bhFetchAll("search/Candidate", {
        query: 'isDeleted:0 AND status:"Placed"',
      ...
      const cands = (r.data || []).map(c => ({
        ...
        title: c.occupation || "", cert: c.customText1 || "", grade: c.customText6 || "", status: c.status || "",
      }));
      answer = `Found **${cands.length}** placed consultants:`;
      data = cands;
    ...
    } else if (question.match(/expir|ending\s+soon|contracts?\s+(ending|coming\s+(off|up))|coming\s+off\s+contract/)) {
      const in90 = nowMs + 90 * 86400000;
      const r = await bhFetchAll("query/Placement", {
        where: `(status = 'Approved' OR status = 'Actively On Contract') AND dateEnd >= ${nowMs} AND dateEnd <= ${in90} AND (employmentType IS NULL OR (employmentType <> 'Direct Hire' AND employmentType <> 'Permanent'))`,
        fields: "id,candidate,jobOrder,dateEnd,payRate,clientBillRate",
        orderBy: "dateEnd",
      });
      ...
      answer = `**${placements.length}** placements expiring in the next 90 days:`;
      data = placements;
      ...
      const r = await bhFetchAll("query/Placement", { where: `(status = 'Approved' OR status = 'Actively On Contract') AND dateEnd >= ${nowMs}`, fields: "id,candidate,jobOrder,clientBillRate", });
      const clientCount = {};
      (r.data || []).forEach(p => {
      ...
      if (count > 0) {
        const avgMargin = ((totalBill - totalPay) / totalBill * 100).toFixed(1);
...
require("./digest")(app, { db: db, graphFetch: graphFetch, outlookUsers: function () { return _outlookUsers; }, getUser: getUser });
